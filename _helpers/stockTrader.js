		...
		for (var i = 0; i < positions.length; i++) {
			...
			if (p.owned) {
				...
				if ((ns.stock.getPrice(p.symbol)) > (p.avgPriceLong * sellForProfitOf)) {
					...
					ns.print(`\t\t..:: Sold ${p.symbol} @ ${soldAt}\tBought@ ${p.avgPriceLong} = ` + money(((soldAt - p.avgPriceLong) * shares) * .99) + `  Profit => ${Math.round(((preSalePrice / p.avgPriceLong)) * 100) / 100}%`);
					netProfit += p.profit;
				...
				if (p.forecast > purchaseForecase) {
					...
					if (cost < ns.getPlayer().money - 100000 - moneyBuffer) {
						...
						ns.print(`\t\t..:: Bought ${p.symbol} @ ${preBuyPrice} x ${p.maxShares} for ${money(cost + 100000)}`);
						netProfit -= cost;
						updateProfit(cost * -1);
						printProfit = true;
						let tmpPositions = updatePositions(ns, symbols);
