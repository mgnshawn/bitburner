...
export async function main(ns) {
    ...
    var baseRemainder = 90;
	 if(ns.args[0] !== undefined && (ns.args[0] == "-h" || ns.args[0] == '-?')) {
        await ns.tprint("Crime it up: auto||(1:shoplift,2:Rob store,3:Mug someone,4:Homicide,4A:Homicide+Assassinate,5:Larceny) optional:moneyRun optional(l longterm, s shortterm, N=run 50 loops, forever but pause 4 seconds so you can kill it");
...
async function chooseBestCrime(ns) {
    ...
    if(ns.getPlayer().strength >= 90) {
        crimeChoice = "Homicide";
    }
